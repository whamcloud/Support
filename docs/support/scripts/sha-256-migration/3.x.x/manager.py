...
def update_agent_bootstrap():
    try_shell([
        'sed',
        '-ie',
        's/csr = launch_command("openssl req -new -subj \/C=\/ST=\/L=\/O=\/CN=%s \
-key %s" % (fqdn, PRIVATE_KEY))/csr = launch_command("openssl req -new -sha256 -subj \
\/C=\/ST=\/L=\/O=\/CN=%s -key %s" % (fqdn, PRIVATE_KEY))/g',
        '/usr/share/chroma-manager/agent-bootstrap-script.template'
    ])
